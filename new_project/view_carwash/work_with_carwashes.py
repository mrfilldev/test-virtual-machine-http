...
def create_prices(request, dict_of_form):
    ...
    for j in dict_of_form:
        if 'price' in j:
            print(j.split('_'))
            if request.form[j] != '':
                prices.append(PricesCarWash(id=j.split('_')[1], category=j.split('_')[2], cost=request.form[j]))
            elif request.form[j] == '':
...
def update_carwash_obj(request, carwash_id):
    ...
    set_fields = {'$set': {
        ...
        'Price': json.loads(json.dumps(create_prices(), default=lambda x: x.__dict__)),
