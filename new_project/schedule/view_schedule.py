    ...
    for i in orders:
        ...
        events_list.append({
            ...
            'start_format': '' if order_obj.DateStart == '' else datetime.strptime(
                (order_obj.DateStart + '.0').replace('Z', ''), "%Y-%m-%dT%H:%M:%S.%f").strftime('%H:%M'),
            'end_format': '' if order_obj.DateEnd == '' else datetime.strptime(
                (order_obj.DateEnd + '.0').replace('Z', ''), "%Y-%m-%dT%H:%M:%S.%f").strftime('%H:%M'),
            'date_format': '' if order_obj.DateCreate == '' else datetime.strptime(
                (order_obj.DateCreate + '.0').replace('Z', ''), "%Y-%m-%dT%H:%M:%S.%f").strftime('%H:%M'),
